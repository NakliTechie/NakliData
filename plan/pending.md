...
- [x] **W7.1** — Sidecar Job 7: `assign-type` — column → semantic type from the FULL taxonomy vocabulary (complements Job 1 `disambiguate-type`, which only ranks existing detector candidates). Covers `unknown` columns via a per-column "Ask AI to classify" button + a schema-toolbar "Classify N unknowns" bulk action. Parser-side hallucination guard (id must be in catalog). (Shipped 2026-06-05; DECISIONS; amendment A24.)
- [x] **W7.2** — Sidecar Job 8: `nl-to-schema` — plain-English dataset description → typed schema (columns + DuckDB types + mapped semantic-type ids). New `nl-to-schema-modal.ts` shows a spec table + CREATE TABLE preview; inserts an un-run DDL cell (Hard NOT #4). "Infer schema" toolbar button. (Shipped 2026-06-05; DECISIONS; amendment A24.) — *bigset evaluation outcome; bigset itself declined as a dep.*
