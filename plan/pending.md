...
- [ ] **W3.4a** — `compute-bridge` source kind in NakliData (this repo, mockable). `src/core/bridge/bridge-client.ts` (health / listTables / query→Arrow IPC, injected fetchImpl) + mount flow + Bearer via source-secrets + graceful "unreachable → Reconnect" fallback + `.naklidata` round-trip (URL + table selection, not token). Per `compute-bridge-protocol.md` §"Client side". Buildable + test-against-a-mock now; real end-to-end needs the binary.
