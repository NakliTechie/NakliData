...
async function handleAction(action: string, el: HTMLElement | null): Promise<void> {
  ...
  switch (action) {
    ...
    case 'ask-nl-to-schema': {
      await openNlToSchemaSidecar(engine);
      return;
    }
    case 'export-html': {
      const wb = workbook.get();
...
/**
 * Sidecar wave 7 W7.2 — Job 10: NL → schema. Opens the modal with the
 ...
 */
async function openNlToSchemaSidecar(engine: Engine): Promise<void> {
  const knownTypes = buildTypeCatalog().map((c) => ({
    typeId: c.typeId,
    displayName: c.displayName,
  }));
  const { openNlToSchemaModal } = await loadChunk('sidecar-ontology');
  openNlToSchemaModal({
    knownTypes,
    onInsert: (ddl) => {
      const nb = getNotebook(engine);
...
/**
 * Sidecar wave 7 W7.1 — Job 9: assign a semantic type to ONE unknown
...
async function runAssignType(
  ...
  try {
    const stats = await engine.sampleColumn(table.name, columnName);
    const settings = await loadSettings();
    const { dispatchOntologyJob } = await loadChunk('sidecar-ontology');
