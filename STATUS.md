## Last update: 2026-05-19T14:00:00Z
## Current milestone: **Classifier integration of user types shipped** — user-defined types now fire as auto-detection candidates during classification, not just as application targets via Override. New `src/taxonomy/user-types.ts` synthesises detectors per UserType; worker holds an `effectiveBundle` updated by a `set_user_types` message; main.ts pushes on workbook change. "Re-classify with user types" button in the schema-panel toolbar preserves user accepts/overrides on rerun. DECISIONS 2026-05-19 14:00. Earlier: AI sidecar arc complete (all three spec §4.3 jobs) on 2026-05-18; Theme 2 + Theme 3 wave 2 + v1.0.0 tag on 2026-05-17. Next: Theme 4 (quality polish) / Theme 1 wave 3 (test infra) / v1.0 carryover / custom-endpoint sidecar.
## Build status: green — `dist/index.html` 372 KB; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 273 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; tsc clean; biome 0 errors / 14 warnings; **132 vitest + 19 Playwright e2e** passing; headless smoke green.
