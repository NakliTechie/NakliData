## Last update: 2026-05-29T23:45:00Z
## Current milestone: **W3.4a landed — Compute Bridge source kind, client side (DECISIONS 23:30; spec amendment A12).** New `'compute-bridge'` SourceKind: paste a bridge URL + Bearer (optional) + local table name + SQL → bridge runs the heavy scan in-VPC → returns Arrow IPC → registered as a local DuckDB table via the new `Engine.registerArrowBuffer`. New `src/core/bridge/bridge-client.ts` (health / listTables / query→ArrayBuffer, injected fetchImpl, mock-tested). Browser uses HTTP + Arrow IPC, NOT Flight (browsers can't do native gRPC). Health-check handshake before SQL; reload-time failures route to `reconnectNeeded` (graceful). Bearer via the W2.2 source-secrets module; `bridge` field on `PersistedSource` (additive, no format-version bump); SQL re-runs on reload (fresh data). **Earlier Wave 3 today:** W3.1 Job 4 (recommend-reports), W3.2 slice A (local-model seam), W3.3 wire-protocol design. **Prior session (2026-05-24):** all of Wave 2 + v1.1.0 + Wave 1 close. **Deferred:** W3.2 slice B (real local inference — needs real browser + WebGPU), W3.4b (multi-table picker via /v1/tables), the bridge BINARY (separate multi-week OSS repo), W2.1c (Iceberg OAuth2 + SigV4), Job 4 LoRA. **Next:** Wave 3 is substantially complete on the NakliData-repo side; remaining work is separate-repo (bridge binary) or real-browser (local model slice B). Good moment for windup.
