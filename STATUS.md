## Last update: 2026-05-23T23:15:00Z
## Current milestone: **Theme 1 wave 3 — DuckDB extensions vendored, JSONL mount now offline-clean.** `scripts/fetch-duckdb-extensions.mjs` (postinstall) downloads `json` (680 KB) + `sqlite_scanner` (1.6 MB) into `public/duckdb-extensions/v1.1.1/wasm_eh/`; engine.boot({offline:true}) sets `custom_extension_repository` to the local path. Smoke now asserts ≥4 tables (was tolerant ≥3 — the JSONL access log used to silently fail). SQLite mount stays bundle-unwired: extension loads but ATTACH fails on duckdb-wasm's VFS (real limitation, fixture lives at `tests/e2e/fixtures/sample-data/finance.sqlite` for future). Excel + read_stat extensions deferred (not published at extensions.duckdb.org for our wasm revision). Sidecar-flow e2e #2 race fixed (classification-stable wait). DECISIONS 2026-05-23 23:00. Earlier: Theme 4 complete on 2026-05-21; classifier integration of user types on 2026-05-19; AI sidecar arc complete on 2026-05-18; Theme 2 + Theme 3 wave 2 + v1.0.0 tag on 2026-05-17. Next: v1.1 tag / Iceberg + S3 precursors / custom-endpoint sidecar / v1.0 carryover.
## Build status: green — `dist/index.html` 408 KB; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 273 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **156 vitest + 25 Playwright e2e** passing; headless smoke green.
