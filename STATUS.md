## Last update: 2026-05-24T22:15:00Z
## Current milestone: **Wave 3 opened — W3.1 Job 4 (report-template recommendation) landed (DECISIONS 22:00; spec amendment A10).** A fourth sidecar job ranks the ALREADY-applicable report templates by fit. Structured output only (`{ recommendations: [{template_id, score}] }`); parser drops unknown ids (hallucination guard), clamps + sorts. Surfaced as an opt-in "Ask sidecar to rank" button in the Suggested-reports panel (shown when sidecar enabled + ≥2 templates apply); ranking reorders cards + adds a fit-score badge. Ranking is ephemeral (cleared on workbook change); templates still instantiate as un-run cells. Eval harness extended with the 4th job (recommend-reports fixture + scorer; dry-run now 42/42). Ships the prompted-base baseline the v1.3 LoRA work will compare against. **Earlier today:** all of Wave 2 (slices 1, 2, 3a, 3b, W2.3, W2.4 + amendments A5–A9). **Deferred to v1.3:** W2.1c (Iceberg OAuth2 + SigV4), Job 4 LoRA specialization. **Next:** W3.2 (local-model path — Transformers.js + Phi-3-mini) or W3.3 (Compute Bridge MVP scaffolding) — both multi-week arcs.
## Build status: green — `dist/index.html` **444 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean (incl. `eval/`); biome 0 errors / 14 warnings; **258 vitest (19 files) + 31 Playwright e2e (20 files)** passing; eval harness dry-run 42/42 (4 jobs); headless smoke green; under 600 KB shell budget.
