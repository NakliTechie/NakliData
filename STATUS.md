## Last update: 2026-05-18T19:00:00Z
## Current milestone: **AI sidecar arc complete for v1.1** — all three spec §4.3 jobs live: wave 1 (BYOK + explain-query-error), wave 2 (type-disambiguation), **wave 3 (define-new-type assist)** + per-workbook user types persisted via the existing `.naklidata` `user_types` field. DECISIONS 2026-05-18 19:00. Theme 2 + Theme 3 wave 2 + v1.0.0 tag landed yesterday. Next: Theme 4 (quality polish) / Theme 1 wave 3 (test infra) / v1.0 carryover / custom-endpoint sidecar / classifier integration of user types.
## Build status: green — `dist/index.html` 372 KB; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 273 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; tsc clean; biome 0 errors / 14 warnings; **123 vitest + 19 Playwright e2e** passing; headless smoke green.
