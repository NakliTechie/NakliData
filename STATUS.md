## Last update: 2026-05-24T22:45:00Z
## Current milestone: **W3.2 slice A landed — local-model sidecar seam (DECISIONS 22:30; spec amendment A11).** A `'local'` SidecarProvider for an in-browser model (no API key, no inference egress). New `src/core/sidecar/local-runtime.ts` registry decouples dispatch from the future model chunk; `dispatchJob` skips the key requirement for `'local'` and routes to the registered generator. Privacy-first: an unloaded local model throws an actionable error rather than silently falling back to a cloud provider (divergence from pending.md "silent fallback" — logged in A11). Settings persistence accepts `'local'`; the Settings RADIO + the actual Transformers.js chunk + inference are **slice B, deferred** (need a real browser + WebGPU — can't be smoke-tested headless). Library chosen: `@huggingface/transformers` (lazy chunk only). **Earlier today:** W3.1 Job 4; all of Wave 2 (slices 1, 2, 3a, 3b, W2.3, W2.4 + amendments A5–A9). **Deferred:** W3.2 slice B (real local inference), W2.1c (Iceberg OAuth2 + SigV4), Job 4 LoRA. **Next:** W3.2 slice B in a session with a real browser, OR W3.3 (Compute Bridge MVP scaffolding).
