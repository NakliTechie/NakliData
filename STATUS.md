## Last update: 2026-05-24T16:45:00Z
## Current milestone: **Wave 2 slice 3b landed — Iceberg REST Catalog navigation (DECISIONS 16:30; spec amendment A8).** New `'iceberg-catalog'` SourceKind mounts an Iceberg table via `(catalogUrl, namespace, table)`. Self-contained REST client (`src/core/iceberg/rest-client.ts`) implements the four endpoints we need (`/v1/config`, list namespaces, list tables, load table); nested namespaces collapse via U+001F. `mountIcebergCatalog` resolves `metadata-location` then reuses the slice 3a engine path. Re-mount via reload re-resolves through the catalog so fresh snapshots pick up automatically. Bearer-only auth in this slice; OAuth2 device flow + AWS SigV4 stay queued for v1.3. Earlier today: slice 3a (Iceberg table-by-URL); slice 2 (S3 endpoints + per-source BYOK secrets); slice 1 (public URL mount + CSP `https:`); Chunk 4 (doc cadence + format-version policy + modal a11y); post-v1.1.0 `applyLoadedFile` mutex; v1.1.0 tag + 27-commit changelog + README v1.1 refresh. **Next:** W2.3 — custom-endpoint sidecar (OpenAI-compatible local llamafiles / vLLM); the CSP `https:` rework from slice 1 has already cleared the runway.
## Build status: green — `dist/index.html` **436 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **218 vitest (17 files) + 31 Playwright e2e (20 files)** passing; headless smoke green; under 600 KB shell budget.
