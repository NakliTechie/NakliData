## Last update: 2026-05-31T12:00:00Z — Wave 5 W5.2 (Sidecar Job 6: result-summary cards) landed. Hex-Magic-style one-line observation card on SQL/cohort/assertion cells. New job `summarise-result` joins the 5-strong sidecar union with prompt builder + parser + hallucination guard (backticked column refs must exist in the result; entire response dropped otherwise) + 200-char cap with ellipsis truncation. UI: "Summarise" trigger button next to row-count meta on successful results, only visible when `app-sidecar-enabled` is on; result renders in the existing `cell-sidecar-result` slot. 8-case eval fixture (incl. hallucination-drop + over-cap truncation + fenced JSON), 12 unit tests, eval gate 50/50 (was 42/42), smoke green. Three of five Wave-5 items now landed (W5.3 aggregation suggestions + W5.4 sensitivity labels + W5.5 assertion cell yesterday; W5.2 today). Two left: W5.2 ✓, W5.1 (NL→SQL Job 5) pending.
