## Last update: 2026-05-21T15:30:00Z
## Current milestone: **Theme 4 wave 1 (column-profile panel) shipped** — schema panel now exposes a per-column Profile button that, on click, runs a full-table aggregate via `Engine.profileColumn` (rows, distinct, null, length min/max/avg, top-5) and renders an inline `.schema-profile-pane` under the column row. Cache lives in `main.ts` as derived state (not in `.naklidata`). Also drops `tests/e2e/fixtures/sample-data/places.geojson` for future spatial-extension tests. DECISIONS 2026-05-21 15:30. Earlier: classifier integration of user types on 2026-05-19; AI sidecar arc complete (all three spec §4.3 jobs) on 2026-05-18; Theme 2 + Theme 3 wave 2 + v1.0.0 tag on 2026-05-17. Next: Theme 4 B2–B4 (side-by-side compare / type-override learns / demo+censor mode) / Theme 1 wave 3 (vendor DuckDB extensions) / v1.0 carryover / custom-endpoint sidecar.
## Build status: green — `dist/index.html` 386 KB; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 273 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; tsc clean; biome 0 errors / 14 warnings; **132 vitest + 20 Playwright e2e** passing; headless smoke green.
