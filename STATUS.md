## Last update: 2026-05-24T21:45:00Z
## Current milestone: **Wave 2 COMPLETE — W2.4 sidecar eval harness landed (DECISIONS 21:30).** New `eval/` harness scores the three sidecar jobs deterministically (rubric-based, no LLM-judge): disambiguate-type (exact typeId), define-type (category + functional regex check), explain-error (keyword coverage + fix check). Reuses the app's exported prompt builders + parsers; bundled for Node via esbuild (no new dep — Node's native type-stripping rejects the codebase's TS parameter properties). Two modes: live (provider key from env) + `--dry-run` (scores recorded fixtures offline, doubles as a self-test). Self-contained HTML report. `eval/` now under tsc + biome discipline. Caught + fixed a latent tsc error in `tests/sidecar-custom-endpoint.test.ts` (committed in W2.3; slipped because that gate ran vitest after the last check, not check last). **All of Wave 2 shipped:** slice 1 (public URL + CSP `https:`), slice 2 (S3 endpoints + per-source BYOK), 3a (Iceberg by-URL), 3b (Iceberg REST catalog), W2.3 (custom-endpoint sidecar), W2.4 (eval harness), W2.5 (spec amendments A5–A9). **Deferred to v1.3:** W2.1c (Iceberg OAuth2 + SigV4). **Next:** Wave 3 — W3.1 Job 4 (report-template recommendation) is the smallest opener; or W3.2 local-model path / W3.3 Compute Bridge MVP scaffolding.
## Build status: green — `dist/index.html` **440 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean (now incl. `eval/`); biome 0 errors / 14 warnings; **243 vitest (19 files) + 31 Playwright e2e (20 files)** passing; eval harness dry-run 34/34; headless smoke green; under 600 KB shell budget.
