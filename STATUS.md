## Last update: 2026-05-24T15:30:00Z
## Current milestone: **Wave 2 slice 1 landed — public URL mount + CSP `connect-src` broadens to `https:` (DECISIONS 15:00; spec amendment A5).** The latent `'http'` SourceKind is now wired end-to-end: `Engine.registerUrl({ tableName, url, format })` does `CREATE OR REPLACE VIEW ... AS SELECT * FROM read_<format>('<url>')`; `mountUrl(engine, opts)` handles validation + classification + persistence round-trip; "Paste URL" button is no longer disabled; new `src/ui/mount-url-modal.ts` (with W1.11 focus management baked in) captures URL + optional label. CSP rework: explicit-host whitelist replaced by `'self' https:` — trade-off acknowledged (broader exfiltration vector if SHA-pinned script-src is bypassed, mitigated by user-explicit URL authorization). Slices 2 (S3 endpoints) and 3 (Iceberg REST catalogs) build on this foundation without further CSP changes. Earlier today: Chunk 4 (W1.9 doc cadence + W1.10 format-version policy + W1.11 modal a11y); post-v1.1.0 `applyLoadedFile` mutex; v1.1.0 tag + 27-commit changelog + README v1.1 refresh. **Next:** Wave 2 slice 2 — S3-compatible endpoint mounting (BYOK secrets pattern mirrored from sidecar).
## Build status: green — `dist/index.html` **412 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **173 vitest (15 files) + 28 Playwright e2e (18 files)** passing; headless smoke green; under 600 KB shell budget.
