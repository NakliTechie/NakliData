## Last update: 2026-05-24T15:45:00Z
## Current milestone: **Wave 2 slice 2 landed — S3-compatible endpoint mounting + per-source BYOK secrets (DECISIONS 15:30; spec amendment A6).** New `'s3-endpoint'` SourceKind connects to AWS S3, MinIO, Cloudflare R2, Backblaze B2, Wasabi — DuckDB's `httpfs` extension does SigV4 natively in the browser, so the v1.1 Relay primitive isn't needed for the common case. New `src/core/secrets/source-secrets.ts` mirrors the sidecar BYOK pattern (sessionStorage default + opt-in IDB plaintext + Forget). Secrets are never persisted in `.naklidata` files; endpoint config (`endpoint`, `region`, `bucket`, `path_prefix`, `url_style`) round-trips as an additive optional field — no format-version bump. New `src/ui/mount-s3-modal.ts` follows the slice 1 modal pattern with focus management baked in. Limitation: DuckDB's `SET s3_*` is connection-wide, so a session can only hold one set of S3 credentials at a time (future enhancement: `CREATE SECRET`). Earlier today: slice 1 (public URL mount + CSP `https:`); Chunk 4 (doc cadence + format-version policy + modal a11y); post-v1.1.0 `applyLoadedFile` mutex; v1.1.0 tag + 27-commit changelog + README v1.1 refresh. **Next:** Wave 2 slice 3 — Iceberg REST catalogs (Bearer / OAuth2 / SigV4 auth chain; reuses source-secrets + the CSP `https:` allowance).
## Build status: green — `dist/index.html` **420 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **195 vitest (16 files) + 30 Playwright e2e (19 files)** passing; headless smoke green; under 600 KB shell budget.
