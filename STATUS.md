## Last update: 2026-05-24T11:50:00Z
## Current milestone: **Wave 1 polish + v1.0 review carryover landed.** Pie chart (custom SVG arc renderer; Plot doesn't ship a pie) and faceted small-multiples (third "facet-by" picker; Plot uses native `fy`, pie partitions into a grid) shipped — commit `983827f`. All four W1.3 v1.0 carryover items closed: CM6 EditorView dispose loop in `notebook.load()` (memory leak fix), `tests/sri-integrity.test.ts` (manifest ↔ vendored bytes round-trip incl. negative tamper), save-load 5/5 stable, taxonomy editorial pass (CIN regex bug fixed, `iso_country_code` `iso3` header noise removed, UUID added → 41 types) — commit `bc78d4a`. **Bonus:** latent CSP-hash bug in `esbuild.config.mjs` fixed (function-form replacers — `String.prototype.replace` was interpreting `$&` in the minified body as the matched substring; the Wave 1 additions tipped the bundle across the threshold where `$&` first appeared) — commit `7a73bc4`. **W1.4 (mirror) deferred** — NakliData's multi-file build doesn't match the existing single-file mirror pattern; needs a deploy workflow + sync-script extension first. Earlier: Theme 1 wave 3 vendored extensions on 2026-05-23; Theme 4 complete on 2026-05-21; classifier integration of user types on 2026-05-19; AI sidecar arc complete on 2026-05-18; Theme 2 + Theme 3 wave 2 + v1.0.0 tag on 2026-05-17. **Next:** W1.2 README v1.1 refresh → W1.1 v1.1.0 tag → Wave 2 (Iceberg + S3 precursors + custom-endpoint sidecar + eval harness).
## Build status: green — `dist/index.html` **413 KB** (+5 KB for pie + facet + CM6 dispose); `dist/chunks/codemirror.js` 370 KB lazy; `dist/chunks/observable-plot.js` **280 KB** (+7 KB for facet `fy` wiring) lazy; `dist/chunks/cytoscape-graph.js` 443 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **165 vitest + 27 Playwright e2e** passing; headless smoke green.
## Branch state: single `main` branch. `v1.0.0` tag pushed. Repo path is moving from `~/Code/naklios-universe/NakliData/` to `~/Code/Apps/NakliData/` (immediately after this docs commit; new session resumes from the new path).
## Deploy status: not yet deployed; tag is the release source-of-truth. W1.4 mirror + GH Pages deploy queued post-v1.1.0.
