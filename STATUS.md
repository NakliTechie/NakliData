## Last update: 2026-05-31T13:00:00Z — **Wave 5 complete (5/5).** W5.1 (Sidecar Job 5: NL → SQL) landed. Hex Magic / Databricks Genie / Snowflake Cortex pattern — user question + workbook schema → DuckDB SQL pre-filled into a new SQL cell. **Never auto-executed** (Hard NOT #4). Parser layers: strip ```sql``` fences → require leading SELECT or WITH → reject any write/DDL keyword (INSERT/UPDATE/DELETE/CREATE/DROP/ALTER/TRUNCATE/MERGE/CALL/ATTACH/COPY/EXPORT/VACUUM/PRAGMA) → hallucination guard (every `FROM <ident>` and `JOIN <ident>` must reference a real table; CTE names + `cell_<id>` shorthand allowed). UI: "✦ Ask in plain English" toolbar button (in the cell-add row, hidden when sidecar is off) opens a modal with question + generated-SQL preview + "Insert as new SQL cell". Only table names + column names are shipped — no row data. 10-case eval fixture (top-vendors, CTEs, quoted identifiers, write-drops, hallucinated-table-drops, prose-drop, empty-drop, fenced strip), 17 unit tests. Eval gate now 60/60 across 6 jobs. **Earlier today (2026-05-31):** W5.2 (Job 6: result-summary cards), W5.3 (aggregation suggestions in schema panel), W5.4 (sensitivity labels), W5.5 (assertion cell kind) all landed. Wave 5 took ~6.5 hr as estimated. Bundle 498.9 KB / 600 KB (83.1%, 101 KB headroom). 317 vitest + 60 eval + headless smoke all green.
