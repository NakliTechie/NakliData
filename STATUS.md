## Last update: 2026-05-29T23:30:00Z
## Current milestone: **Wave 3 in progress — W3.1 (Job 4) + W3.2 slice A (local-model seam) + W3.3 design landed today (2026-05-29).** (1) W3.1: `recommend-reports` sidecar job ranks already-applicable templates; structured output only; opt-in "Ask sidecar to rank" in the Suggested-reports panel; eval harness extended to 4 jobs (dry-run 42/42). Spec A10; DECISIONS 22:00. (2) W3.2 slice A: `'local'` SidecarProvider + `src/core/sidecar/local-runtime.ts` registry seam + dispatch routing (skips key requirement) + settings persistence + tests; privacy-first explicit-not-silent fallback; library `@huggingface/transformers` (lazy chunk only); **slice B (real model) deferred — needs a real browser + WebGPU**. Spec A11; DECISIONS 22:30. (3) W3.3: Compute Bridge wire-protocol design — [`plan/compute-bridge-protocol.md`](plan/compute-bridge-protocol.md); browser uses HTTP + Arrow IPC (not Flight); results via existing `registerArrow`; **the binary is a separate multi-week repo**. **Prior session (2026-05-24):** all of Wave 2 (slices 1, 2, 3a, 3b, W2.3, W2.4 + amendments A5–A9) + v1.1.0 + Wave 1 close. **Deferred:** W3.2 slice B, W3.4a (bridge client seam — buildable here, mockable), W2.1c (Iceberg OAuth2 + SigV4), Job 4 LoRA. **Next:** W3.4a (compute-bridge source kind, mockable) — the one Wave-3 piece still buildable + verifiable in this repo.
## NOTE on dates: this conversation spanned 2026-05-24 → 2026-05-29. Git timestamps are authoritative. Some earlier-written DECISIONS/amendment cross-references for today's work may carry "2026-05-24" labels from following the in-repo date; the W2.4/W3.1/W3.2A/W3.3 commits are git-dated 2026-05-29.
## Build status: green — `dist/index.html` **444 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean (incl. `eval/`); biome 0 errors / 14 warnings; **263 vitest (20 files) + 31 Playwright e2e (20 files)** passing; eval harness dry-run 42/42 (4 jobs); headless smoke green; under 600 KB shell budget.
