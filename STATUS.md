## Last update: 2026-05-21T17:00:00Z
## Current milestone: **Theme 4 complete** — B1 (column-profile panel, wave 1) earlier today + B2 (compare-tables modal), B3 (type override learns), B4 (demo / censor mode) just shipped (wave 2). Schema panel now offers Compare-tables… + Override-rules management + a Settings → Demo mode toggle. `Engine.compareTables` runs a FULL OUTER JOIN bucket aggregate + column-level diff sample; `workbook.overrideRules` persists "always treat columns named X as Y" rules to `.naklidata`; `maskLabel` in `src/core/demo-mode.ts` swaps user-data labels for stable `src_1/tbl_1/col_1` tokens across the sources panel, schema panel, and SQL result headers. DECISIONS 2026-05-21 17:00. Earlier: column-profile panel on 2026-05-21 morning; classifier integration of user types on 2026-05-19; AI sidecar arc complete on 2026-05-18; Theme 2 + Theme 3 wave 2 + v1.0.0 tag on 2026-05-17. Next: Theme 1 wave 3 (vendor DuckDB extensions) / v1.0 carryover / custom-endpoint sidecar / enterprise Iceberg + S3-compatible precursors.
## Build status: green — `dist/index.html` 408 KB; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 273 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; tsc clean; biome 0 errors / 14 warnings; **156 vitest + 24 Playwright e2e** passing; headless smoke green.
