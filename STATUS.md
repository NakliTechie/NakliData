## Last update: 2026-05-24T16:15:00Z
## Current milestone: **Wave 2 slice 3a landed — Iceberg table-by-URL with optional Bearer auth (DECISIONS 16:00; spec amendment A7).** New `'iceberg-table'` SourceKind mounts a single Iceberg table via its metadata.json URL. `Engine.configureIceberg({ bearerToken })` installs the `iceberg` extension and sets `extra_http_headers` for Bearer auth. `Engine.registerIcebergTable({ tableName, metadataUrl })` creates the view via `iceberg_scan('<url>')`. Smart table-name derivation handles the canonical `<table>/metadata/v<N>.metadata.json` layout. Bearer tokens use the same BYOK pattern as S3 secrets (sessionStorage default + opt-in IDB plaintext + Forget); empty/whitespace token = public table, nothing saved. **Slice 3b deferred** — REST catalog navigation + OAuth2 device flow + AWS SigV4 (for Glue) is its own sitting. Earlier today: slice 2 (S3 endpoints + per-source BYOK secrets); slice 1 (public URL mount + CSP `https:`); Chunk 4 (doc cadence + format-version policy + modal a11y); post-v1.1.0 `applyLoadedFile` mutex; v1.1.0 tag + 27-commit changelog + README v1.1 refresh. **Next:** Wave 2 slice 3b — Iceberg REST catalog browser + OAuth2 + SigV4 (gnarliest of Wave 2 — own sitting).
## Build status: green — `dist/index.html` **428 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **203 vitest (16 files) + 31 Playwright e2e (20 files)** passing; headless smoke green; under 600 KB shell budget.
