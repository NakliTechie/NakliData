## Last update: 2026-05-24T13:00:00Z
## Current milestone: **Post-v1.1.0 cleanup: `applyLoadedFile` re-entrancy fixed via promise-chain mutex; e2e save-load reverts the IDB-clear workaround and now exercises the race directly (DECISIONS 2026-05-24 13:00).** The v1.1.0 e2e fix sidestepped the race in the test by clearing IDB between save and reload — the underlying production bug (boot-time auto-restore racing an explicit Load → 4 source cards instead of 2) was left as a follow-up. This change adds a module-level `_applyLoadedChain` in `src/main.ts` that serialises all three callers (lens decode, snapshot restore, user Load); the public function signature is unchanged. Earlier today (2026-05-24T12:35Z): README v1.1 refresh + v1.1.0 release notes drafted, two e2e stability fixes, **W1.4 (mirror) deferred** to its own short wave after the tag. Earlier: Wave 1 polish + v1.0 review carryover landed 2026-05-24; Theme 1 wave 3 vendored extensions on 2026-05-23; Theme 4 complete on 2026-05-21; classifier integration of user types on 2026-05-19; AI sidecar arc complete on 2026-05-18; Theme 2 + Theme 3 wave 2 + v1.0.0 tag on 2026-05-17. **Next:** tag `v1.1.0` (ask before pushing) → Wave 2 (Iceberg + S3 precursors + custom-endpoint sidecar + eval harness).
## Build status: green — `dist/index.html` **413 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **165 vitest (15 files) + 26 Playwright e2e (17 files)** passing; headless smoke green; under 600 KB shell budget.
## Branch state: single `main` branch, pushed to origin. `v1.0.0` tag at `5b10b93`; `v1.1.0` tag pending user approval to push. Repo path is `~/Code/Apps/NakliData/`.
