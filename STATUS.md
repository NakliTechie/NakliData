## Last update: 2026-05-24T14:45:00Z
## Current milestone: **Chunk 4 small follow-ups landed.** W1.9: doc-cadence pinned in CLAUDE.md (windup summaries are canonical going forward; pre-windup `checkpoint-*-eod.md` files stay as historical record). W1.10: `.naklidata` format-version bump policy documented (DECISIONS 14:00 — additive optional fields don't bump; required-field changes do). W1.11: `src/ui/schema-graph.ts` a11y audit — moved focus to close button on open, restore focus to trigger on close, fixed Escape-listener leak (previously self-removed only on Escape; backdrop/X closes leaked). E2e schema-graph spec gains two focus-behavior assertions. W1.4 (naklios.dev mirror) dropped 2026-05-24 — NakliData positions as an independent product, not a launcher app; cross-repo `sync-mirrors.sh` work is no longer on the roadmap. W1.8 (GH Pages deploy) deferred — we're nowhere near needing a hosted build yet. Earlier today (2026-05-24T13:00Z): post-v1.1.0 `applyLoadedFile` mutex fix. Earlier still: v1.1.0 tag + 27-commit changelog + README v1.1 refresh. **Next:** Wave 2 — Iceberg + S3 custom endpoints (Chunk 1) OR custom-endpoint sidecar + eval harness (Chunk 2).
## Build status: green — `dist/index.html` **404 KB**; `dist/chunks/codemirror.js` 364 KB lazy; `dist/chunks/observable-plot.js` 276 KB lazy; `dist/chunks/cytoscape-graph.js` 436 KB lazy; `dist/chunks/maplibre-map.js` 1.0 MB lazy; `dist/sw.js` 2.7 KB; vendored extensions ~2.3 MB under `dist/duckdb-extensions/` (not precached); tsc clean; biome 0 errors / 14 warnings; **165 vitest (15 files) + 26 Playwright e2e (17 files)** passing; headless smoke green; under 600 KB shell budget.
