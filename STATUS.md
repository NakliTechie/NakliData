...
## Current milestone: **W3.4b landed — Compute Bridge catalog picker (DECISIONS 2026-05-30; spec amendment A12 W3.4b follow-up).** New `'compute-bridge-catalog'` SourceKind: paste bridge URL + Bearer → Connect probes `/v1/health` + lists `/v1/tables` → user multi-selects tables with per-table row caps → each pick materialised as `SELECT * FROM "<name>" LIMIT <cap>` via the bridge. All picks land under one MountedSource with N MountedTables. Per-table failures are non-fatal (other tables still mount). Persistence shape distinct from W3.4a single-SQL (`bridge_catalog` field tracks `{name, local_name, row_cap}[]`). 6 new vitest specs (happy path, default cap, identifier escaping, partial failures, all-fail, validations). Modal uses two-phase reveal (Connect → table list with checkboxes + cap inputs → Mount selected). New shell button. applyLoadedFile branch re-fetches the saved selection on reload. Smoke modal-cycle now covers 6 modals (was 5). **Earlier today (2026-05-30):** the F+G+H+I+K polish slate — README refresh for Wave 2/3 surfaces, e2e focus a11y assertions for 4 older modals (+ robust `restoreModalFocus` helper that survives panel re-renders), stop-checklist ordering + spec-amendments index, smoke modal-open guard, bundle-size gate. **2026-05-29 main work:** W3.1 Job 4 (recommend-reports), W3.2 slice A (local-model seam), W3.3 wire-protocol design, W3.4a bridge client. **Prior session (2026-05-24):** all of Wave 2 + v1.1.0 + Wave 1 close. **Deferred:** W3.2 slice B (real local inference — needs real browser + WebGPU), the bridge BINARY (separate multi-week OSS repo), W2.1c (Iceberg OAuth2 + SigV4), Job 4 LoRA. **Next:** Wave 3 is functionally complete on the NakliData-repo side; remaining work is separate-repo (bridge binary) or real-browser (local model slice B).
