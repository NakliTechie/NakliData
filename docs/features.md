...
NakliData reads tabular data from local files/folders, HTTPS, and S3-compatible
storage. Bytes move directly between the named source and the browser; there is
no NakliData relay. CORS or signed-URL access still applies.
...
**Unavailable in this build:** Iceberg REST Catalog remains disabled until an
exact Databricks Unity, Snowflake Horizon, or eligible legacy Open
Catalog/Polaris matrix passes.
...
Off by default. Settings → AI sidecar to enter a key. Ten narrow jobs, each with
...
adapter and reports the selected device; WASM is the fallback. Local generation
remains experimental pending the complete structured-output quality matrix.
