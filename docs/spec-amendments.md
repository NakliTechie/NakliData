...
| [A27](#a27--incremental-refresh-v12-m3-amends-spec-41--38) | §4.1 + §3.8 | User-initiated source change detection. FSA metadata and HTTP validators establish per-session baselines; canonical source lineage drives staged, atomic remounts and dependency-ordered recomputation. A changed baseline advances only after the complete refresh succeeds. |
...
**Amended wording:** A user-initiated **Check changes**
mechanism (header button next to Lineage).
On click:

1. **Per source, compute current fingerprint:**
   - **FSA folder**: aggregate supported-file size and maximum
     modification time plus a stable hash of sorted
     `filename + size + lastModified` metadata. No file contents
     enter the fingerprint.
     Permission check is opportunistic (queryPermission only — no
     prompt); revoked permission, a partial scan, or an oversized
     scan marks the source uncheckable.
   - **HTTP URL**: HEAD request → ETag + Last-Modified +
     Content-Length headers. A reachable response with none of these
     validators is uncheckable, not evidence that the source is current.
   - **S3-endpoint / Iceberg / Bridge**: uncheckable until a real
     change token is implemented.
...
2. **Diff against persisted fingerprint map** (IDB; key
   ...
   The first successful observation is persisted immediately as the
   baseline; it is not called fresh or current because there was no
   prior observation.
3. **Canonicalize and cascade source IDs** via the M2 lineage graph:
   old table-name lineage snapshots are migrated to stable mounted
   `source.id` values, then BFS walks forward from each stale source
   → cell IDs whose result is
...
4. **Surface the diff** in a result modal showing:
   ...
   - Affected cells in topological dependency order.
   - Couldn't check (permission, network, partial scan, unsupported
     source, or missing HTTP validator).
   - First baselines recorded this pass.
   - "No detectable source changes" when no difference is observed.
5. **On user confirm**:
   - read and validate every changed FSA/HTTP source into
     collision-safe staging relations;
   - swap existing relations and remove missing folder relations in
     one DuckDB transaction;
   - publish the reconciled source metadata, preserving stable source
     and table IDs;
   - run every affected cell sequentially in topological order;
   - persist the new fingerprint only when every remount and cell run
     returns success.

   `Notebook.runCell()` returns `success`, `failure`, `cancelled`, or
   `not-runnable`. Any non-success retains the previous fingerprint
   baseline, so the next check proposes the change again.
...
- **NO persistence of file contents** — only fingerprint tokens
  (size, lastModified, metadata hash, ETag, etc.).
...
**Status:** Initial M3 shipped in v1.2 (`42be469`). The
commit-on-success rebuild shipped in Goal Phase 2 on 29 July 2026.
Regression coverage includes canonical identity and legacy migration,
HTTP validator absence, first-baseline creation, DuckDB swap rollback,
discriminated cell outcomes, dependency ordering, and the full
baseline → changed bytes → staged remount → changed result lifecycle.
