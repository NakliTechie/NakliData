...
required-endpoint enforcement, and the invariant that the product's Iceberg
source entry points remain unavailable.
...
authentication, authorization, storage access, token refresh, vendor-specific
errors, rate limits, or cancellation against a live service.
